...
public class ConvertServiceImpl implements IConvertService {
	...
	@Override
	public void doConvert(DataFile dataFile) throws DataException {
		
	}

	@Override
	public void convertFlow(DataTable table, List<Integer> successIds, List<Integer> failedIds, String tableName)
	...
	public void rollBack(DataFile dataFile) throws DataException {
		try {
			commonDao.deleteByUnId(dataFile.getDataFileType().getTableConvert().getTableName(), dataFile.getUnId());
			dataFile.setStatusConvert(DataFile.DATA_STATUS_NOTSTART);
			BaseDataEntity baseDataEntity =new BaseDataEntity();
			baseDataEntity.setTableName(dataFile.getDataFileType().getTableClean().getTableName());
	...
	public void updateFileStatus(int ope, DataFile dataFile) throws DataException {
		try {
			switch (ope) {
			case 0:
				dataFile.setStatusConvert(DataFile.DATA_STATUS_STARTED);
				break;
			case 1:
				dataFile.setStatusConvert(DataFile.DATA_STATUS_FAILED);
				break;
			case 2:
				dataFile.setStatusConvert(DataFile.DATA_STATUS_FINISHED);
				if(TurnOnOff.getInstance().isAuto()){
					dataFile.setStatusValidate(DataFile.DATA_STATUS_READY);
				}
				break;
			case 4:
				dataFile.setStatusConvert(DataFile.DATA_STATUS_PAUSE);
				break;
			default:
				dataFile.setStatusConvert(DataFile.DATA_STATUS_NOTSTART);
				break;
	...
	public DataFile getDataFile(String fileId) throws DataException {
        ...
        try {
            ...
            TableDef table = tableDefDao.getById(df.getDataFileType().getTableConvertId());
            df.getDataFileType().setTableConvert(table);
        }
        catch (Exception e) {
            e.printStackTrace();
