...
public class ValidateServiceImpl implements IValidateService {
	...
	@Override
	public void doValidate(DataFile dataFile) throws DataException {
	}

	@Override
	public void validateFlow(DataTable table, List<Integer> successIds, List<Integer> failedIds, String tableName)
	...
	public void rollBack(DataFile dataFile) throws DataException {
		try {
			commonDao.deleteByUnId(dataFile.getDataFileType().getTableValidate().getTableName(), dataFile.getUnId());
			dataFile.setStatusConvert(DataFile.DATA_STATUS_NOTSTART);
			BaseDataEntity baseDataEntity =new BaseDataEntity();
			baseDataEntity.setTableName(dataFile.getDataFileType().getTableConvert().getTableName());
			...
			e.printStackTrace();
			throw new DataException("ETL-ORA0051","Update the data filed.");
		}
	...
	public void updateFileStatus(int ope, DataFile dataFile) throws DataException {
		try {
			switch (ope) {
			case 0:
				dataFile.setStatusValidate(DataFile.DATA_STATUS_STARTED);
				break;
			case 1:
				dataFile.setStatusValidate(DataFile.DATA_STATUS_FAILED);
				break;
			case 2:
				dataFile.setStatusValidate(DataFile.DATA_STATUS_FINISHED);
				break;
			case 4:
				dataFile.setStatusValidate(DataFile.DATA_STATUS_PAUSE);
				break;
			default:
				dataFile.setStatusValidate(DataFile.DATA_STATUS_NOTSTART);
				break;
			}
			dataFileDao.update(dataFile);
		} catch (Exception e) {
	...
	public DataFile getDataFile(String fileId) throws DataException {
        ...
        try {
            ...
            TableDef table = tableDefDao.getById(df.getDataFileType().getTableValidateId());
            df.getDataFileType().setTableValidate(table);
        }
        catch (Exception e) {
            e.printStackTrace();
