...
public class VerifyTask implements Runnable {
	...
	private final static Logger logger = LoggerFactory.getLogger(VerifyTask.class);
	
	private final static String VERIFY_AUTOEXECUTOR_NAME = "verifyAutoExecutor";
	private final static String VALIDATE_SERVICE_NAME = "validateService";
	...
	private IValidateService validateService = null;
	private VerifyAutoExecutor verifyAutoExecutor = null;
	...
	public VerifyTask(DataFile df, TaskDetailPojo vo) {
		this.dataFile = df;
		this.vo = vo;
	}
	...
	@Override
	public void run() {
		logger.info("~~~~Start validating the ["+dataFile.getFileName()+"] file.~~~~");
		try {
			...
			if(vo.isStop()){
    			...
    			verifyAutoExecutor.removeMap(String.valueOf(dataFile.getEtlDatafileId()));
        		return;
        	}
			//根据源table、目标table查询规则模板，再根据模板查询具体处理器
			getRules();
			
			validateService.updateFileStatus(0, dataFile);
			if(logger.isDebugEnabled()){
				logger.debug("Successfully modified the file named ["+dataFile.getFileName()+"] in the state of validating.");
			}
            ...
            if(totalCount == 0){
            	throw new DataException("ETL0030","According this ["+dataFile.getUnId()+"] fileId, data that needs to be validated is not found in the convert table.");
            }
            logger.info("The file ["+dataFile.getFileName()+"] has ["+totalCount+"] pieces of data that need validating.");
            //将总条数更新到vo对象中
            ...
            for (; i < forCount; i++) {
        		...
        		if(listPage == null || listPage.size() == 0){
        			throw new DataException("ETL0030","No data was found to be validated.");
        		}
            	...
            	//将清洗完毕的数据插入清洗表
            	validateService.validateFlow(dataTable, successIds, failedIds,dataFile.getDataFileType().getTableConvert().getTableName());
            ...
            if(totalCount == successCount){
            	validateService.updateFileStatus(2, dataFile);
            	saveLog("Success");
            	if(logger.isDebugEnabled()){
			...
			try {
				validateService.updateFileStatus(1, dataFile);
		...
		logger.info("~~~~End validating the ["+dataFile.getFileName()+"] file.~~~~");
	}
	...
	private void saveLog(String msg) throws DataException{
		try {
			...
			dataFileLog.setAction(DataFile.ACTION_VALIDAT);
	...
	private void initServices() throws DataException {
		...
		verifyAutoExecutor = SpringContextHolder.getBean(VERIFY_AUTOEXECUTOR_NAME);
		validateService = SpringContextHolder.getBean(VALIDATE_SERVICE_NAME);
		...
		if(verifyAutoExecutor == null){
			throw new DataException("ETL0030","The [verifyAutoExecutor] is null.");
		}
	...
	private List<Map<String,Object>> listPage(BasePage page) throws DataException{
		try {
			return commonService.listByPage(dataFile.getDataFileType().getTableConvert(), page);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private int queryCount() throws DataException{
		try {
			return commonService.getCountByUnIdAndResult(dataFile.getDataFileType().getTableConvert().getTableName(),dataFile.getUnId(), BaseDataEntity.RESULT_PENDING);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private DataTable process(List<Map<String,Object>> listData) throws DataException{
		try {
			...
			if((dt.getHeader().size() -2) != dataFile.getDataFileType().getTableValidate().getColumns().size()){
				throw new DataException("ETL0038","Field mapping error.tablename is ["
				+ dataFile.getDataFileType().getTableConvert().getTableName()+"],"
				+ "Target tablename is [" + dataFile.getDataFileType().getTableValidate().getTableName()+"].");
			}
	...
	private DataTable addColumn(){
		...
		dt.setTableName(dataFile.getDataFileType().getTableValidate().getTableName());
	...
	private String clean(List<RuleOp> ruleOps,String value) throws DataException{
		try {
			...
			e.printStackTrace();
			throw new DataException("ETL9999","Make a unknown exception when validating.");
		}
	}
	...
	private void getRules() throws DataException{
		try {
			TableDef tbConvert = tableService.getById(dataFile.getDataFileType().getTableConvertId());
			if(tbConvert == null || tbConvert.getColumns() == null || tbConvert.getColumns().size() == 0){
				throw new DataException("ETL-ORA0040","No data was found according to the convert table id.");
			}
			dataFile.getDataFileType().setTableConvert(tbConvert);
			TableDef tbValidate = tableService.getById(dataFile.getDataFileType().getTableValidateId());
			if(tbValidate == null || tbValidate.getColumns() == null || tbValidate.getColumns().size() == 0){
				throw new DataException("ETL-ORA0040","No data was found according to the validate table id.");
			}
			dataFile.getDataFileType().setTableValidate(tbValidate);
			if(logger.isDebugEnabled()){
				logger.debug("Queryed convert table and validate table end.");
				logger.debug("convert tablename -->["+dataFile.getDataFileType().getTableConvert().getTableName()+"]. it has ["+dataFile.getDataFileType().getTableConvert().getColumns().size()+"] columns.");
				logger.debug("validate tablename -->["+dataFile.getDataFileType().getTableValidate().getTableName()+"]. it has ["+dataFile.getDataFileType().getTableValidate().getColumns().size()+"] columns.");
			}
			this.listRuleUse = ruleUseService.listRuleUseByTables(dataFile.getDataFileType().getTableConvertId(), dataFile.getDataFileType().getTableValidateId());
			if(listRuleUse == null || listRuleUse.size() == 0){
				throw new DataException("ETL-ORA0041","No rule mapping was found according to the convert table id and validate table id.");
			}
	...
	private List<RuleOp> parseJson(String json) throws DataException{
		try {
			...
			for (Object object : parseArray) {
				...
				CleanParam[] cleanParams = validateService.parseParams(ruleOp.getParams());
				if(cleanParams == null){
