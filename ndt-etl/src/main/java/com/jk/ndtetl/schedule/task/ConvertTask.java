...
public class ConvertTask implements Runnable {
	private final static Logger logger = LoggerFactory.getLogger(ConvertTask.class);
	private final static String TRANSFORM_AUTOEXECUTOR_NAME = "transFormAutoExecutor";
	private final static String CONVERT_SERVICE_NAME = "convertService";
	...
	private IConvertService convertService = null;
	private TransFormAutoExecutor transFormAutoExecutor = null;
	...
	public ConvertTask(DataFile df, TaskDetailPojo vo) {
		this.dataFile = df;
		this.vo = vo;
	}
	...
	@Override
	public void run() {
		logger.info("~~~~Start Converting the ["+dataFile.getFileName()+"] file.~~~~");
		try {
			...
			if(vo.isStop()){
        		...
        		transFormAutoExecutor.removeMap(String.valueOf(dataFile.getEtlDatafileId()));
        		return;
        	}
			//根据源table、目标table查询规则模板，再根据模板查询具体处理器
			getRules();
			
			convertService.updateFileStatus(0,dataFile);
			if(logger.isDebugEnabled()){
				logger.debug("Successfully modified the file named ["+dataFile.getFileName()+"] in the state of converting.");
			}
            ...
            if(totalCount == 0){
            	throw new DataException("ETL0030","According this ["+dataFile.getUnId()+"] fileId, data that needs to be converted is not found in the clean table.");
            }
            logger.info("The file ["+dataFile.getFileName()+"] has ["+totalCount+"] pieces of data that need converting.");
            //将总条数更新到vo对象中
            ...
            for (; i < forCount; i++) {
        		...
        		if(listPage == null || listPage.size() == 0){
        			throw new DataException("ETL0030","No data was found to be converted.");
        		}
            	...
            	//将清洗完毕的数据插入清洗表
            	convertService.convertFlow(dataTable, successIds, failedIds,dataFile.getDataFileType().getTableClean().getTableName());
            ...
            if(totalCount == successCount){
            	convertService.updateFileStatus(2,dataFile);
            	saveLog("Success");
            	if(logger.isDebugEnabled()){
                	logger.debug("The log was logged successfully.");
                }
            	logger.info("Converting file ["+dataFile.getFileName()+"] successfully");
            }else{
            	if(failedCount > 0){
                	throw new DataException("ETL0040",errorMsg.toString());
                }
            	throw new DataException("ETL0040","The number of successes is not equal to the total.");
            }
		} catch (DataException e) {
			logger.error(e.errorDesc());
			try {
				convertService.updateFileStatus(1,dataFile);
		...
		logger.info("~~~~End Converting the ["+dataFile.getFileName()+"] file.~~~~");
	}
	...
	private void saveLog(String msg) throws DataException{
		try {
			...
			dataFileLog.setAction(DataFile.ACTION_CONVERT);
			...
			dataFileLog.setRunning(dataFile.getStatusConvert());
	...
	private void initServices() throws DataException {
		...
		transFormAutoExecutor = SpringContextHolder.getBean(TRANSFORM_AUTOEXECUTOR_NAME);
		convertService = SpringContextHolder.getBean(CONVERT_SERVICE_NAME);
		...
		if(transFormAutoExecutor == null){
			throw new DataException("ETL0030","The [transFormAutoExecutor] is null.");
		}
	...
	private List<Map<String,Object>> listPage(BasePage page) throws DataException{
		try {
			return commonService.listByPage(dataFile.getDataFileType().getTableClean(), page);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private int queryCount() throws DataException{
		try {
			return commonService.getCountByUnIdAndResult(dataFile.getDataFileType().getTableClean().getTableName(),dataFile.getUnId(), BaseDataEntity.RESULT_PENDING);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private DataTable process(List<Map<String,Object>> listData) throws DataException{
		try {
			...
			if((dt.getHeader().size() -2) != dataFile.getDataFileType().getTableConvert().getColumns().size()){
				throw new DataException("ETL0038","Field mapping error.tablename is ["
				+ dataFile.getDataFileType().getTableClean().getTableName()+"],"
				+ "Target tablename is [" + dataFile.getDataFileType().getTableConvert().getTableName()+"].");
			}
			...
			e.printStackTrace();
			throw new DataException("ETL9999","Make a unknown exception where process convert.");
		}
	}
	...
	private DataTable addColumn(){
		...
		dt.setTableName(dataFile.getDataFileType().getTableConvert().getTableName());
	...
	private String clean(List<RuleOp> ruleOps,String value) throws DataException{
		try {
			...
			e.printStackTrace();
			throw new DataException("ETL9999","Make a unknown exception when converting.");
		}
	}
	...
	private void getRules() throws DataException{
		try {
			...
			if(tbClean == null || tbClean.getColumns() == null || tbClean.getColumns().size() == 0){
				throw new DataException("ETL-ORA0040","No data was found according to the clean table id ["+dataFile.getDataFileType().getTableCleanId()+"].");
			}
			...
			if(tbConvert == null || tbConvert.getColumns() == null || tbConvert.getColumns().size() == 0){
				throw new DataException("ETL-ORA0040","No data was found according to the conver table id ["+dataFile.getDataFileType().getTableConvertId()+"].");
			}
			dataFile.getDataFileType().setTableConvert(tbConvert);
			if(logger.isDebugEnabled()){
				logger.debug("Queryed clean table and convert table end.");
			...
			this.listRuleUse = ruleUseService.listRuleUseByTables(dataFile.getDataFileType().getTableCleanId(), dataFile.getDataFileType().getTableConvertId());
			if(listRuleUse == null || listRuleUse.size() == 0){
				throw new DataException("ETL-ORA0041","No rule mapping was found according to the clean table id and convert table id.");
			}
	...
	private List<RuleOp> parseJson(String json) throws DataException{
		try {
			...
			for (Object object : parseArray) {
				...
				CleanParam[] cleanParams = convertService.parseParams(ruleOp.getParams());
				if(cleanParams == null){
