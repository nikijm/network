...
public class PreSystemThread extends Thread {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				// 获取第一个未处理记录进行处理
				MBusinessProcessSync mBusinessProcessSync = preSystemService.getNTopOneUnProcessedM();
				System.out.println("---------------------------------------------------------------");
				...
				if (null != mBusinessProcessSync) {
					...
					HttpPost httpPost = new HttpPost("http://localhost:8080/etl/plat/jinrong/apply/receive");
